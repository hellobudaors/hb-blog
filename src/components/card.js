        ...
        if (node) {
            ...
            if (this.props.cardtype === `first-on-index`) {
                ...
                if (node.data.post_type === `Kiemelt`) {
                    card =
                        <Link to={ `/posts/${node.slugs[0]}` } className={ colSpan + `pa4 db link dim flex flex-column white bg-dark-gray` }>
                    ...
                    card =
                        <Link to={ `/posts/${node.slugs[0]}` } className={ colSpan + `pa4 db link dim flex flex-column white bg-dark-gray justify-between` } style={ cardStyle }>
                    ...
                    card =
                        <Link to={ `/posts/${node.slugs[0]}` } className={ cardClass + colSpan + `pa4 db link dim flex flex-column white` }>
                    ...
                    card =
                        <Link to={ `/posts/${node.slugs[0]}` } className={ colSpan + `pa4 db link dim flex flex-column bt b--black black bg-near-white` }>
                            <h2 key={ node.id } className="pa0 ma0 lh-title barlow-condensed f2 word-wrap measure">{ node.data.title.text }</h2>
                            <p className="pa0 ma0 mt3 mb4 lh-copy flex-auto overflow-hidden sans-serif dark-gray f5 word-wrap measure">{ node.data.excerpt.text }</p>
